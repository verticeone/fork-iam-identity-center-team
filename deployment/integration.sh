...
amplifyAppId=`aws amplify list-apps --region $REGION --output json | jq -r '.apps[] | select(.name | test("team"; "i")) | .appId'`
amplifyDomain=`aws amplify list-apps --region $REGION --output json | jq -r '.apps[] | select(.name | test("team"; "i")) | .defaultDomain'`
amplifyDomain="sandbox.$amplifyDomain"
...
if [ -n "$amplifyCustomDomain" ]; then
  amplifyCustomDomainPrefix=$(echo $amplifyCustomDomains | jq -r 'select(.domainAssociations | length > 0) | .domainAssociations[0].subDomains[] | select(.subDomainSetting.branchName=="sandbox") | .subDomainSetting.prefix')
